...
//
// 14-05 双模式同断言双跑（D-01，蓝本 PITFALLS :382 handshake 行「协议守卫与
// 进程模型无关」）：七测全部经 newTestServer 小族 t.Run("mode=shared"/
// "mode=per-client") 双跑——两列执行同一断言体且期望值与改造前逐字一致
//（子协议双闸/半开闸/5s 超时/抢跑/version_mismatch 全部发生在升档分岔之前，
// ro INPUT 门控两模式同链）。唯一分叉 = TestReadOnlyAllowsResize 的 ro 运行期
// RESIZE 处置（D-09 第二闸 shared 忽略 / D-06 per-client 直通——server.go
// RESIZE case 两分支真值相反）+ 其 exitf 收口面，按 14-01 断言分叉表形态处理
//（shared 列期望值逐字未动，per-client 列断言直通真值——非「两模式都接受」
// 的削弱形态，PITFALLS P11 红线）。超时护栏一律 10s ctx。
...
func TestHalfOpenPerIP429(t *testing.T) {
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			exitCh, wsURL := newTestServer(t, mode, []string{"/bin/cat"}, func(o *server.Options) { o.MaxHalfOpenPerIP = 1 })
	...
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			exitCh, wsURL := newTestServer(t, mode, []string{"/bin/cat"}, func(o *server.Options) {
				o.HelloTimeout = 200 * time.Millisecond
			})
...
//
// 14-05 双跑注记：ro INPUT 门控两模式同链（server.go INPUT case 的 mode 门为
// 共用代码）；RESIZE 写通断言只观测「写路径不报错」——per-client 列该 RESIZE
// 经 D-06 直通落自己 PTY（TIOCSWINSZ 对 /bin/cat 无可观测输出），本测断言面
// 两列同值。
//
...
func TestReadOnlyDropsInput(t *testing.T) {
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			...
			ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
			defer cancel()
			c, wmode := dialHello(t, ctx, wsURL, 80, 24)
			if wmode != proto.ModeRO {
				t.Fatalf("welcome mode = %q, want %q (writable=false)", wmode, proto.ModeRO)
			}
			...
			// 放行语义）——忽略 ≠ 关连接，写路径不报错（per-client 列：D-06 直通落
			// 自己 PTY，同样无输出可观测，写成功即存活证据）。
...
//
// 14-05 断言分叉表（D-02，蓝本 handshake 行的唯一分叉面）：ro 运行期 RESIZE 的
// 处置两模式真值相反——shared = D-09 第二闸忽略（本测 v1.0 断言逐字：stty#2
// "44 111"）；per-client = D-06 直通本会话 PTY（server.go RESIZE case per-client
// 分支：ro/rw 同形直通，ttyd parity——protocol.c 只门 INPUT 不门 RESIZE），50ms
// 防抖后 TIOCSWINSZ(120,50) 生效，stty#2 读出 "50 120"。两列各断言本模式真值
// 全强度（per-client 列断言 resize 确实到达——比「忽略」更强的可证伪面），
// 非「两模式都接受」的削弱形态（PITFALLS P11 红线）；per-client 侧同语义的
// 白盒面另由 TestPerClientResizePassthroughRO（perclient_test.go 12-02）承载。
// exitf 收口面同 TestExitFrameSignal（14-01）先例分叉：shared = waitExit(0)，
// per-client = 静默（无第二终结源，PC-02/03）。
//
...
// per-client 列同夹具零漂移：spawn 发生在 attach 期（Hello 之后），无 pre-attach
// drain 窗口，前导休眠仅推迟 stty#1 输出（10s 护栏内余量充足）。
func TestReadOnlyAllowsResize(t *testing.T) {
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			argv := []string{"/bin/sh", "-c", "sleep 0.5; stty size; sleep 1; stty size"}
			exitCh, wsURL := newTestServer(t, mode, argv, func(o *server.Options) { o.Writable = false })

			ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
			defer cancel()
			c, wmode := dialHello(t, ctx, wsURL, 111, 44)
			if wmode != proto.ModeRO {
				t.Fatalf("welcome mode = %q, want %q (writable=false)", wmode, proto.ModeRO)
			}
			...
			// 首个 stty：Hello 携 (cols=111, rows=44) 生效（两模式同断言——shared 经
			// 仲裁参与集 / per-client 经 StartWithSize 出生即正确尺寸，SC1 后半）。
			rows, cols := readSize()
			if rows != "44" || cols != "111" {
				t.Fatalf("first stty size = %q %q, want \"44 111\" (Hello-carried size)", rows, cols)
			}

			// ro 下发 RESIZE {cols:120, rows:50}——断言分叉表见函数头注释。
			resize, err := json.Marshal(struct {
				Cols int `json:"cols"`
				Rows int `json:"rows"`
			}{Cols: 120, Rows: 50})
			...
			// 第二个 stty：断言分叉表（D-02）——shared = "44 111"（D-09 第二闸忽略，
			// 原断言逐字）；per-client = "50 120"（D-06 直通 + 50ms 防抖后生效）。
			wantRows, wantCols := "44", "111"
			if mode == server.SessionModePerClient {
				wantRows, wantCols = "50", "120"
			}
			rows, cols = readSize()
			if rows != wantRows || cols != wantCols {
				t.Fatalf("second stty size = %q %q, want %q %q（ro RESIZE：%s）", rows, cols, wantRows, wantCols,
					map[bool]string{true: "per-client D-06 直通生效", false: "ignored since 05-04, D-09 second gate"}[mode == server.SessionModePerClient])
			}

			// sh 随夹具结束退出 → D-10：lifecycle 发 1000 正常关闭帧（客户端读取循环
			// 使库自动回显关闭帧，服务端 Close 握手不等超时）——per-client 列同值
			//（sessionWatcher 私有 EXIT + Close(1000)，EXIT 帧被本读循环按数据帧收集
			// 后继续，CloseError 断言面同构）。
			var ce websocket.CloseError
			for {
				if _, _, rerr := c.Read(ctx); rerr != nil {
					if !errors.As(rerr, &ce) {
			...
			// exitf 收口分叉（TestExitFrameSignal 14-01 先例同款）：shared = 0
			//（原断言逐字）；per-client = 静默（无第二终结源）。
			if mode == server.SessionModePerClient {
				assertNoExit(t, exitCh)
				return
			}

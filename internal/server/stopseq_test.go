...
// 经 New 兜底 SIGHUP）承接。
//
// 夹具纪律（2026-08-26 本机 setsid 探针实证）：
...
//     onChunk 丢弃不可观测，落盘标记是信号类断言的既定形态）。
//   - `trap "" TERM; sleep 100` 不是恒活夹具——TERM 杀死 sleep 后 sh 走到
//     脚本尾正常退出（探针实测 exit 0）；KILL 补发测的恒活形态必须是
//     `while :; do sleep 10; done` 循环（TERM 只杀当轮 sleep，sh 忽略后
//     进入下一轮——探针实测 TERM 后存活、KILL 后 137）。
//
...
// （`trap "" TERM` + while 循环恒活夹具，探针实证形态）→ stop-timeout 到期
// AfterFunc 异步补发 SIGKILL → 信号死亡 → exitf(-1)（accept-255 同常量）。

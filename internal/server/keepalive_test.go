...
//
// 14-05 双模式同断言双跑（D-01，蓝本 PITFALLS :382 keepalive 行「保活与进程
// 模型无关」）：三测经 newTestServer 小族 t.Run 双跑，两列执行同一断言体且
// 期望值与改造前逐字一致——pinger 是每连接 goroutine（shared Attach 尾部 /
// per-client startSessionGoroutines 同件同参数），与进程模型零耦合。
//
...
func TestPingDisabled(t *testing.T) {
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			...
			// 100ms pong 超时必触发断开，后续 echo 必然失败。PingInterval 显式置 0
			// 镜像原 Options 字面（小族基线即零值——显式写出锁定「0 = 禁用」语义，
			// 防基线漂移时本测静默变质）。
			exitCh, wsURL := newTestServer(t, mode, []string{"/bin/cat"}, func(o *server.Options) {
				o.PingInterval = 0

...
// 纪律延伸到测试输出面）。
//
// 14-06 双跑（蓝本 PITFALLS :383 basepath 行 mode-agnostic 同断言）：三测
// for-mode + t.Run 双跑（startBasePathServer 已随 Task 1 前批 mode 参数化），
// 两列同一断言体期望值逐字未动。
...
func TestBasePathWS(t *testing.T) {
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			_, bpWSURL := startBasePathServer(t, mode, func(o *server.Options) {

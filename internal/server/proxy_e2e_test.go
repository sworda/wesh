...
//
// 14-06 双跑（蓝本 PITFALLS :383 proxy* 行 mode-agnostic 同断言）：auth-header/
// XFF 信任闸与进程模型无关——四测 for-mode + t.Run 双跑；5 处装配调用点按用途
// 分流（:46/:218/:281 消费 waitHandlers stderr 同步边 → newTrackedTestServer；
// :127/:179 丢弃 waitHandlers → newTestServer），XFF 信任闸/remote_user sanitize
// 红线断言两列同值逐字未动。
...
func TestRemoteUserLogging(t *testing.T) {
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			...
			_, wsURL, waitHandlers := newTrackedTestServer(t, mode, []string{"/bin/cat"}, func(o *server.Options) {
				o.Credentials = []server.Credential{cred}
				o.AuthHeader = "X-Remote-User"
			})
...
func TestShareChannelRemoteUser(t *testing.T) {
	for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
		t.Run("mode="+mode, func(t *testing.T) {
			...
			_, wsURL2, waitHandlers2 := newTrackedTestServer(t, mode, []string{"/bin/cat"}, func(o *server.Options) {
				o.Credentials = []server.Credential{cred}
				o.AuthHeader = "X-Remote-User"
				o.ShareTokenRO = roTok
				o.ShareTokenRW = rwTok

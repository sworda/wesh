...
// 明文态结构性不可观测）。
//
// 14-06 双跑（蓝本 PITFALLS :383 customindex 行 mode-agnostic 同断言）：九
// 装配调用点收编 newTestServer / startBasePathServer(mode)（Task 2 参数化）
// 小族——八子测试 for-mode + t.Run 双跑，两列同一断言体期望值逐字未动
//（HTTP 伺服面与进程模型无关；Writable 字段各子测试无观测面，基线 true
// 与原零值在本文件断言集下行为等价——14-05 TestLogRedaction 同款口径）。
...
func TestCustomIndex(t *testing.T) {
		...
		for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
			t.Run("mode="+mode, func(t *testing.T) {
				...
				_, wsURL := newTestServer(t, mode, []string{"/bin/cat"}, func(o *server.Options) {
					o.CustomIndex = []byte(customIndexPage)
	...
	t.Run("security headers same as built-in page", func(t *testing.T) {
		for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
			t.Run("mode="+mode, func(t *testing.T) {
				_, wsURLC := newTestServer(t, mode, []string{"/bin/cat"}, func(o *server.Options) {
					o.CustomIndex = []byte(customIndexPage)
				})
				_, wsURLB := newTestServer(t, mode, []string{"/bin/cat"}, nil)
	...
	t.Run("base-path combo serves custom page", func(t *testing.T) {
		for _, mode := range []string{server.SessionModeShared, server.SessionModePerClient} {
			t.Run("mode="+mode, func(t *testing.T) {
				httpBase, _ := startBasePathServer(t, mode, func(o *server.Options) {
					o.CustomIndex = []byte(customIndexPage)
				})

...
#   （--credential 一次性测试凭据，贴近生产形态——LAN 暴露面仅存在于实证窗口期，T-09-08c）。
...
case "${1:-}" in
  setup)
    ...
    nohup $WESH_BIN --bind 127.0.0.1 --port $WESH_PORT --credential user:pass --writable -- bash --norc --noprofile >$WESH_LOG 2>&1 &
    echo $! > $WESH_PIDF
    sleep 0.8

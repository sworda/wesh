...
- [x] 12-02-PLAN.md — PC-05/PC-07：debouncer 共用件 + RESIZE 直通分支 + ro 放行 D-06/D-07 同 plan 配对 + Go 断言五测

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 12-03-PLAN.md — PC-10/PC-11：outbox 恢复信号 + 阻塞持帧 + dwell 看门狗（10s 常量 + Options 覆写）+ gateTransitions 两递增点 + WR-01 闭合回指 + Go 断言四测

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 12-04-PLAN.md — phase12.mjs 协议层 UAT 六场景（session 双模式对照/resize 隔离/ro 直通/门控限速/停读续读/真实 10s+ dwell 1013）
...
- [x] 13-03-PLAN.md — 终结语义：pcSupervisor/pcExitReq 第二终结源三形态 + last-reaped-code 退出码两时序对齐 + session_end emit + WR-02 waitDone 栅栏（PC-09/OPS-12）

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 13-04-PLAN.md — Shutdown N 进程组快照逐组信号 + 有界 join（含断开未收割残留形态）+ session_end 数==N（PC-09）
- [x] 13-05-PLAN.md — 观测面：metrics 四计数器 series + session_active 模式分支 + HELP 双模式 + healthz 恒 true + session_start emit + 镜像 17→21（OPS-12）

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 13-06-PLAN.md — SEC-09：StartOptions.RemoteUser + whitelistEnv 扩展（空串不出键）+ SpawnFunc 签名扩散链 + remoteUser 到达断言（SEC-09）

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 13-07-PLAN.md — phase13.mjs 协议层 UAT 六场景 + churn 负载格（10rps×30s 双采样差值有界）（PC-08/PC-09/SEC-09/OPS-12）

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 13-08-PLAN.md — 收口闸六段式 + diff 白名单审查 + 四需求勾选 + Key Decisions 登记 + D-10 文档明示段（全需求）
...
- [x] 14-07-PLAN.md — per-client 负载矩阵：TestLoadPerClientFloodMatrix 洪水格 + TestLoadPerClientResident 驻留格（D-10/D-12）+ LOADDATA 产出 + D-11 两段式判定（PC-12）
- [x] 14-08-PLAN.md — phase14.mjs：herdr driving（api snapshot 翻转链 + 流层互证）+ ro 汇聚协议层 UAT（D-05/D-06/D-08/D-09，PC-13）

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 14-02-PLAN.md — mode-mapped 大型批：e2e_test 生命周期八测 + multi_test fanout/容量/计数十一测断言分叉表（PC-13）
- [x] 14-03-PLAN.md — mode-mapped 终结/观测批：emptyexit/shutdown/metrics 三文件 Phase 13 混入归一（第二终结源/N 组信号/session_active 列，PC-13）
- [x] 14-04-PLAN.md — resize 直通分叉 + mode-exclusive 批：resize_arb/clients 的 D-03 显式断言未装配（PC-13）
- [x] 14-05-PLAN.md — mode-agnostic 协议守卫+认证批：handshake/limits/keepalive + auth*/origin/throttle/tickets 八文件同断言双跑（PC-13）
...
- [x] 14-12-PLAN.md — 收口闸六段式 + UAT 矩阵人工闸 + diff 白名单终审 + PC-12/PC-13 勾选 + flagged_assumptions 终验（全需求）
...
| 14. 双模式验证矩阵、标定与 herdr UAT | v1.1 | 12/12 | Complete | 2026-09-08 |

...
- [ ] 13-01-PLAN.md — D-01 one-way 确认门 + stop-timeout 双默认值（per-client 未设默认 5s / 显式 0 尊重+warn / shared 字面 0 不变）+ 显式位第八位 + 三态测试 + 双文档行（PC-08）
...
- [ ] 13-03-PLAN.md — 终结语义：pcSupervisor/pcExitReq 第二终结源三形态 + last-reaped-code 退出码两时序对齐 + session_end emit + WR-02 waitDone 栅栏（PC-09/OPS-12）

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 13-04-PLAN.md — Shutdown N 进程组快照逐组信号 + 有界 join（含断开未收割残留形态）+ session_end 数==N（PC-09）
- [ ] 13-05-PLAN.md — 观测面：metrics 四计数器 series + session_active 模式分支 + HELP 双模式 + healthz 恒 true + session_start emit + 镜像 17→21（OPS-12）

**Wave 4** *(blocked on Wave 3 completion)*

- [ ] 13-06-PLAN.md — SEC-09：StartOptions.RemoteUser + whitelistEnv 扩展（空串不出键）+ SpawnFunc 签名扩散链 + remoteUser 到达断言（SEC-09）

**Wave 5** *(blocked on Wave 4 completion)*

- [ ] 13-07-PLAN.md — phase13.mjs 协议层 UAT 六场景 + churn 负载格（10rps×30s 双采样差值有界）（PC-08/PC-09/SEC-09/OPS-12）

**Wave 6** *(blocked on Wave 5 completion)*

- [ ] 13-08-PLAN.md — 收口闸六段式 + diff 白名单审查 + 四需求勾选 + Key Decisions 登记 + D-10 文档明示段（全需求）

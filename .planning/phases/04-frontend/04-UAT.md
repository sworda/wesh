...
status: pending
phase: 04-frontend
source: [04-06-PLAN.md]
created: 2026-08-18
updated: 2026-08-18
---

# Phase 4 UAT —— 渲染面人工确认清单

协议层断言已全自动化（`node web/uat/phase04.mjs <wesh 二进制>`，十场景：Welcome prefs 形状/注入/theme 对象/osc52 下发/last-wins/启动拒绝矩阵），本清单只列**渲染/浏览器平台行为面**——自动化结构性不可达的部分（沿用 03-UAT.md 分工先例）。
...
### T2. IME 组合输入（FE-02，UI-SPEC 🧪 backstop 人工出口）
...
### T9. 离开确认 beforeunload（FE-06 / D-18；含 sticky activation 裁决记录）
...
### T10. 偏好下发与 query 覆盖（FE-07 / D-16 / D-19）

...
### 观测面最小钩子（Phase 14 粒度的边界划分）
- **D-04:** 仅 **spawn_failed 单行审计事件**本阶段先行（`logEvent(remote, 1011, "spawn_failed", remoteUser)` 形态，零敏感值——Pitfall 5 失败清理清单的测试锁定项）；容量再闸拒绝同通道以区分事件名记录（D-02）。`wesh_pty_spawn_failures_total` counter 归 Phase 14——metricsSeries17 镜像契约保持 Phase 14 一次性扩展（17→N），本阶段 metrics.go 零改动；session_start/session_end 的 per-client 粒度（client_id 关联键）归 Phase 14——Phase 11→14 窗口期 per-client 会话生命周期审计为空白（已知且接受：会话事件对在 Phase 14 一次补齐，避免两半套 schema）

### 验证面切片
- **D-05:** Go 测试 = **新增 `internal/server/perclient_test.go` per-client-only 独立文件**（研究 §11 文件清单形态）；既有 shared 测试零改动原样跑（零回归证据不动）；`newTestServer(t, mode)` 参数化 harness 与 Pitfall 11 三维归类表归 Phase 15 统一收编——本阶段不碰任何既有测试文件装配点
...
- `.planning/STATE.md` §Blockers — v1.1 裁决项归属（① stop-timeout 默认值 → Phase 13；③ healthz/metrics OQ → Phase 14；④ spawn-intent 口径 → 本 phase D-03 提前消解）
...
- `.planning/milestones/v1.0-phases/06-lifecycle/06-CONTEXT.md` — EXIT 帧直写纪律（组帧一次/同步 Write 2s/Close 1000/禁 outbox 异步）与 accept-255 裁决（OQ1；per-client --once 对齐在 Phase 14，本 phase 不涉及）
...
- `internal/server/server.go` — Options.SessionMode/SpawnFunc（:301-309，10-01 已装配）/ ValidateOptions 互斥校验（:337-345）/ New 零值兜底（:392-393）与字段直传（:445-446）/ Attach 升档序列（:807 起；守卫区⓪-③ :749-798；checkTicket :1159；Welcome 恒首帧论证 :961-1043；registerLocked :1024）/ pinger（:1211）/ lifecycle（:1359，per-client 不装配）/ terminate/termOnce（:1446，supervisor 归 Phase 14 复用）/ Shutdown（:1479，本阶段不分支）
- `internal/server/clients.go` — client 结构（:81-146，+inQ/pc 两字段）/ outbox trySend（:178）/ registerLocked/removeLocked（:311/:329，记账单点）/ kickSlowConsumerLocked（:564-606，1013 既有机械）/ detach（:761 附近，SIGHUP 挂点同位）/ maybeExitWhenEmptyLocked（:852，本阶段不分支——Phase 14）/ stopChildLocked（:893，每会话化母本）/ inputQ（defaultInputQueueBytes 容量复用）
...
- `docs/ARCHITECTURE.md` §2.9 等 — v1.0 落地架构对照（双模式文档段归 Phase 15，本阶段不动）
...
- `server.go New 尾部 goroutine 钉死点`（:443-446 附近）— 模式分岔：shared 现状三件套逐字不动；per-client 本阶段零全局 goroutine（pcSupervisor 归 Phase 14），五 goroutine 全部随 attach 每会话装配
...
- **Phase 11→14 窗口期的两处已知语义缺口（明示接受，非疏漏）**：per-client 下 --once/--exit-when-empty 永不退出（Pitfall 1，第二终结源归 Phase 14——本阶段 maybeExitWhenEmptyLocked 不分支，per-client 下该路径显式注释锚定）；per-client 会话无 session_start/session_end 审计（D-04，Phase 14 一次补齐）
...
- **wesh_pty_spawn_failures_total counter** — D-04 裁决归 Phase 14（metricsSeries17 镜像一次性扩展）；EMFILE 级联的 Prometheus 可见性随 Phase 14 落地
- **per-client 会话 session_start/session_end 审计（client_id 关联键）** — Phase 14 本体（D-04 窗口期空白已明示接受）
- **per-client stop-timeout 默认值重议**（裁决项①）— Phase 13：0=不补 KILL 在 per-client = HUP 免疫泄漏，公开契约变更需用户裁决；D-01 机制已就位，届时仅改默认值
- **pcSupervisor / 第二终结源 / --once·exit-when-empty per-client 语义** — Phase 14（Pitfall 1）；本阶段 maybeExitWhenEmptyLocked 不分支
- **newTestServer(t, mode) 参数化 harness 与三维归类表** — Phase 15（Pitfall 11）；perclient_test.go 届时按归类表收编

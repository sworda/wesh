...
//   - D-04：覆盖面 = 26 个长期运行 flag + command exec 数组 = 27 键；
//     no-auth/insecure-http/version/help/config 五逃生门键不入 fileConfig——
//     严格模式（未知键拒绝）将其自然拒绝（逃生门必须显式说出口，配置文件里
//     写出来等于没说）。
...
// fileConfig 是配置文件的解码目标：27 键 = 26 flag 同名 + command。
